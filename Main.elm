 ...
 case ( List.head l ) of 
  Just word ->                       
   case ( List.tail l ) of           
    Just vec -> (word, vec)         
    Nothing ->                        
     ("<unk>", ( List.repeat vec_dim "0.0" ))      
  Nothing -> ("<unk>", ( List.repeat vec_dim "0.0" ))


parse_wordvec wordvec_text = 
 Dict.map (\k a -> (normalize a)) ( Dict.fromList ( List.map convert ( List.map String.words (String.lines wordvec_text) ) ) ) 
